...
#include <string>
#include "raylib.h"
#include "raygui.h" // Se precisar de raygui aqui, mas geralmente � no cpp
#include "Item.h"
#include "Upgrade.h"
...
public:
    ...
    // Configura��es de Texto Flutuante
    const float ftextLifetime = 0.5f;

    // Vari�vel para controle de frames (usada na tela de LOGO)
    int frameCounter;
    ...
    void InitGameContent(); // Fun��o para popular itens/upgrades
    void SaveGame();
    void LoadGame();
    void ResetSave();

    // M�todos auxiliares de desenho (opcional, para organizar o Draw)
    void DrawLogo();
    void DrawMenu();
    void DrawGameplay();
    void DrawShop();
